...
function renderPreset(key) { currentPreset = key; const preset = mobilePresets[key]; mobilePrompt.value = preset.prompt; userResultTitle.textContent = preset.title; userResultText.textContent = preset.text; urlParams.set('scenario', currentPreset); window.history.replaceState({}, '', `${window.location.pathname}?${urlParams.toString()}`); renderSharedFlow(); }
function syncRole() { roleButtons.forEach((button) => { button.classList.toggle('is-active', button.dataset.role === currentRole); }); userView.classList.toggle('is-hidden', currentRole !== 'user'); adminView.classList.toggle('is-hidden', currentRole !== 'admin'); if (currentRole === 'admin') { heroTitle.textContent = '管理员可在手机端快速查看 AI 平台运行状态。'; heroText.textContent = '包括 tokens、模型调用、审批积压、活跃智能体和风险告警。'; mobilePromptLabel.textContent = '移动端联动查询'; runActionBtn.textContent = '查看联动状态'; } else { heroTitle.textContent = '在手机上也能直接发起企业工作请求。'; heroText.textContent = '统一入口承接会议、制度、差旅、权限、采购等工作场景，再由内部智能体分流执行。'; mobilePromptLabel.textContent = '移动端工作请求'; runActionBtn.textContent = '开始处理'; } urlParams.set('role', currentRole); window.history.replaceState({}, '', `${window.location.pathname}?${urlParams.toString()}`); }
function renderSharedFlow() {
  ...
  sharedFlowMeta.innerHTML = [`<span class="meta-pill">当前状态：${flowStore.statusLabels[activeFlow.status] || activeFlow.status}</span>`,`<span class="meta-pill">待审批：${summary.pendingApprovals}</span>`,`<span class="meta-pill">来源：${activeFlow.channel || 'desktop'} / ${activeFlow.role || 'user'}</span>`].join('');
...
runActionBtn.addEventListener('click', () => { const presetKey = inferPreset(mobilePrompt.value); renderPreset(presetKey); const preset = mobilePresets[presetKey]; if (currentRole === 'admin') { flowStore.recordEvent({ scenarioId: presetKey, role: 'admin', channel: 'mobile', action: '在移动端查看联动状态' }); renderSharedFlow(); return; } const metric = scenarioMetrics[presetKey] || { tokens: 26000, cost: 50 }; flowStore.upsertTask({ scenarioId: presetKey, status: preset.status, prompt: mobilePrompt.value, summary: preset.status === 'done' ? `${preset.label}结果已同步到移动端。` : `${preset.label}方案已同步到移动端，等待继续处理。`, channel: 'mobile', role: 'user', approvalCount: preset.status === 'waiting' ? 1 : 0, tokensDelta: metric.tokens, costDelta: metric.cost, action: '移动端发起或继续任务' }); renderSharedFlow(); });
